# ==============================================================================
# Inception Data Analysis - Complete
# ==============================================================================
# 
...
a <- site_h$meta()
a1 <- data.table::data.table(colnames(a), t(a[1, ]))

# Update column names
colnames(a1) <- c("Param", "Data")
...
if (output_format == "PDMforPCs") {
  ...
  write.csv(pdm_data, file = file_name_event, row.names = FALSE)
...
# Initialise an empty list to store each gauge's rainfall data
rainfall_data <- list()
...
for (i in seq_along(use_api)) {
  ...
  rainfall_data[[i]] <- data_rainfall
}

# Combine rainfall series
# Start with the first gauge
combined_rainfall <- rainfall_data[[1]]

# Iteratively join each additional gauge by dateTime
...
weighted_rainfall <- as.data.frame(combined_rainfall)

# Calculate weighted sum for each row
weight_total <- sum(weights)

weighted_rainfall$value <- apply(
  weighted_rainfall[, rainfall_values],
  1,
  function(row) {
    # Keep only non-NA values
    valid <- !is.na(row)
    
    # Calculate normalised weighted sum of available values
    if (any(valid)) {
      # Rescale weights to account for missing gauges
      weight_total * sum(row[valid] * weights[valid]) / sum(weights[valid])
    } else {
      # If all gauges have NA values return 0
      # PDMforPCs doesn't accept NA values
      0
    }
  }
)
...
datetime_lt_weighted <- as.POSIXlt(weighted_rainfall$dateTime, tz = "GMT")

pdm_data_weighted <- data.frame(
...
# Add rainfall data
pdm_data_weighted$rainfall <- weighted_rainfall$value

# Write data to csv file
write.csv(pdm_data_weighted, file = file_name_weighted, row.names = FALSE)

# Clear some objects to save memory
rm(rainfall_data, combined_rainfall, weighted_rainfall)
...
if (!exists("pt_data")) {
  pt_data <- data.table::fread(paste0(base_spol_dir, "PT_raw_data.csv"))
}
...
for (site_id_pt in site_id_list_pt) {
  ...
  s0 <- stringr::str_remove(site_id_pt, "^0+")
  
  # Create a list of the top events
  dt_events <- unique(pt_data[
    WISKI == s0 & Type == event_type & EventID %in% c(1:n_events),
    .(dateTime = Datetime_obs, Observed = Obs, Event = EventID)
  ])
  
  # Rename column headings
  data.table::setnames(dt_events, 1, "Date and Time")
  data.table::setnames(dt_events, 3, "Event Rank")
  
  if (event_type == "Flow") {
    data.table::setnames(dt_events, 2, "Observed Flow (cumecs)")
  } else {
    data.table::setnames(dt_events, 2, "Observed Stage (m)")
  }
  ...
  if (nrow(dt_events) == 0) {
    ...
    loc_name <- unique(pt_data[
      WISKI == s0 & Type == event_type & EventID %in% c(1:1),
      .(location = Location)
    ])
    
    # Remove the needless additional "River " at the start
    loc_name <- stringr::str_remove(loc_name$location, "River ")
    ...
    seasons_pt <- sapply(angles_pt, angle_to_season)
    
    # Combine all the necessary data for the plot into a frame
    df_polar <- data.frame(
    ...
    if (output_event_data) {
      ...
      for (i in 1:length(dt_events$`Date and Time`)) {
        ...
        if (output_format_pt == "PDMforPCs") {
          ...
          # Write data to csv file
          write.csv(pdm_data_event, file = file_name_event_pt, row.names = FALSE)
          
        } else {
          
          # Write in default format
          write.csv(
            event_data_pt$data[, 1:2],
...
for (i in seq_along(site_id_list_yearly)) {
  ...
  if (is.na(wiski_exports_yearly[[i]])) {
    ...
    if (is.na(site_h1$data$dateTime[[length(site_h1$data$dateTime)]] < end_datetime_yearly)) {
      ...
      site_h1$data <- site_h1$data[
        site_h1$data$dateTime >= start_datetime_yearly &
          site_h1$data$dateTime <= end_datetime_yearly,
      ]
    }
  }
  ...
  annual_data_yearly <- list(year0 = NA)
  
  # Loop over hydro years to create a plot for each
  for (yr in (start_year + 1):end_year) {
    
    # Select out data for this year
    yr_data <- site_h1$data[site_h1$data$hydroYear == yr]
    
    # Catch empty data
    if (length(yr_data$dateTime) == 0) {
      annual_data_yearly[[toString(yr)]] <- data.frame(
  ...
  for (yr in (start_year + 1):end_year) {
    ...
    yr_plot <- ggplot2::ggplot(h_data_melt, aes(Time, value, color = Measure)) +
      theme_bw() +
      geom_line(linewidth = 0.75) +
      labs(x = NULL, y = ylabel_yearly) +
      xlab(yr) +
    ...
    plotlist_h[[length(plotlist_h) + 1]] <- yr_plot + theme(legend.position = "none")
  }
...
# Create list that will store annual data
annual_data_cumul <- list(year0 = NA)
new_col_names <- c("Time")
...
for (i in seq_along(raingauge_ids_cumul)) {
  ...
  if (is.na(wiski_exports_cumul[[i]])) {
    ...
    site_r_cumul <- wiski_export_to_r6(
  ...
  for (yr in (start_year_cumul + 1):end_year_cumul) {
    ...
    # Select out data for this year
    yr_data_cumul <- site_r_cumul$data[site_r_cumul$data$hydroYear == yr]
    
    # Make a default data frame if no data exists for this year
    if (length(yr_data_cumul$dateTime) == 0) {
      site_df <- data.frame(
        Time = as.POSIXct(paste0(toString(yr - 1), "-10-01 09:00:00"), tz = "GMT"),
        Rain = NA
      )
    ...
    if (i == 1) {
      # First site: create new data frame
      annual_data_cumul[[toString(yr)]] <- site_df
    } else {
      # Add to existing data frame
      annual_data_cumul[[toString(yr)]] <- base::merge(
...
for (yr in (start_year_cumul + 1):end_year_cumul) {
  ...
  colnames(annual_data_cumul[[toString(yr)]]) <- new_col_names
  
  # Reshape data frame to plot both series together
  r_data_melt <- reshape2::melt(
    annual_data_cumul[[toString(yr)]],
    id.vars = "Time",
  ...
  yr_plot_cumul <- ggplot2::ggplot(r_data_melt, aes(Time, value, color = Raingauge)) +
    theme_bw() +
    geom_line(linewidth = 0.75) +
  ...
  # Generate a legend for the final plot
  plotlist_r[[length(plotlist_r) + 1]] <- yr_plot_cumul + theme(legend.position = "none")
}
  ...
  if (sources[[i]] == "Raingauges") {
    ...
    for (j in seq_along(raingauge_ids_dm[[i]])) {
      ...
      if (is.na(read_files_dm[[i]][[j]])) {
        ...
        site_r_dm <- wiski_export_to_r6(
          file_name = read_files_dm[[i]][[j]],
          measure = "rainfall"
        )
        
        # Reduce onto chosen time range
        if (is.na(site_r_dm$data$dateTime[[length(site_r_dm$data$dateTime)]] < end_datetime_dm)) {
          stop(paste("File", read_files_dm[[i]][[j]], "has empty last row(s). Remove in text editor."))
                   ...
                   site_r_dm$data$dateTime[[length(site_r_dm$data$dateTime)]] < end_datetime_dm) {
          stop(paste("File", read_files_dm[[i]][[j]], "does not span the entire date range."))
        } else {
          site_r_dm$data <- site_r_dm$data[
            site_r_dm$data$dateTime >= start_datetime_dm &
              site_r_dm$data$dateTime <= end_datetime_dm,
          ]
        }
      }
      
      # Construct a data frame with all gauge data
      if (j == 1) {
        # Start new data frame
        df_dm <- data.frame(
          dateTime = site_r_dm$data$dateTime,
          value1 = raingauge_weights_dm[[i]][[1]] * site_r_dm$data$value
        )
      } else {
        # Create temporary data frame to add as a new column
        tempdf_dm <- data.frame(
          dateTime = site_r_dm$data$dateTime,
          value = raingauge_weights_dm[[i]][[j]] * site_r_dm$data$value
        )
        colnames(tempdf_dm) <- c("dateTime", paste0("value", j))
        
        # Combine data frames, sharing the dateTime column
        df_dm <- Reduce(function(x, y) merge(x, y, all = TRUE), list(df_dm, tempdf_dm))
      }
    }
    
    # Sum the contributions from the gauges
    # Note: does not adjust weights if some data is missing
    if (length(raingauge_ids_dm[[i]]) > 1) {
      df_dm <- data.frame(
        dateTime = df_dm$dateTime,
        value = rowSums(df_dm[, 2:(length(raingauge_ids_dm[[i]]) + 1)], na.rm = TRUE)
      )
    }
    ...
    raw_data_dm <- read.csv(
      file = read_files_dm[[i]],
      sep = ",",
      header = FALSE,
      col.names = c("year", "month", "day", "minutes", "date", "time", "Rainfall(mm)", "areal_coverage", "na"),
      row.names = NULL,
      skip = 18
    )
    
    # Define function for constructing the required time format
    build_datetime <- function(dfrow) {
      paste(toString(dfrow[[1]]), "-", toString(dfrow[[2]]), "-", toString(dfrow[[3]]), " ", toString(dfrow[[6]]), sep = "")
    }
    
    # Make data frame with required format
    df_dm <- data.frame(
      dateTime = apply(raw_data_dm, 1, build_datetime) %>%
        as.POSIXct(format = "%Y-%m-%d %H:%M", tz = "GMT"),
      value = as.numeric(raw_data_dm$Rainfall.mm.)
    )
  }
  
  # Store the data frame with the appropriate name
  if (i == 1) {
    df1_dm <- df_dm
  } else {
    df2_dm <- df_dm
...
# Clear df and tempdf to save memory
rm(df_dm, tempdf_dm)
...
if (df1_dm$dateTime[[1]] != df2_dm$dateTime[[1]]) {
  ...
  if (df1_dm$dateTime[[1]] > df2_dm$dateTime[[length(df2_dm$dateTime)]] ||
      df2_dm$dateTime[[1]] > df1_dm$dateTime[[length(df1_dm$dateTime)]]) {
  ...
  df1_dm <- df1_dm[df1_dm$dateTime >= largest_start_time, ]
  df2_dm <- df2_dm[df2_dm$dateTime >= largest_start_time, ]
}

# Convert to cumulative rainfall
df1_dm <- data.frame(
  dateTime = df1_dm$dateTime,
  cumSum1 = riskyData::cumsumNA(df1_dm$value)
)

df2_dm <- data.frame(
  dateTime = df2_dm$dateTime,
  cumSum2 = riskyData::cumsumNA(df2_dm$value)
)

# Merge data frames
fulldf_dm <- Reduce(function(x, y) merge(x, y, all = TRUE), list(df1_dm, df2_dm))

# Remove any entries with NA present
fulldf_dm <- fulldf_dm[!is.na(fulldf_dm$cumSum1) & !is.na(fulldf_dm$cumSum2), ]

# Clear df1 and df2 to save memory
rm(df1_dm, df2_dm)

# Create the double mass plot
# Add column to track multiple lines
fulldf_dm$line <- rep("main", length(fulldf_dm$cumSum1))

# Identify dateTime range used in final plot
datetime_start_dm <- fulldf_dm$dateTime[[1]]
datetime_end_dm <- fulldf_dm$dateTime[[length(fulldf_dm$dateTime)]]

# Set distance between parallel guidelines
dguide <- fulldf_dm$cumSum2[[length(fulldf_dm$cumSum2)]] -
  fulldf_dm$cumSum1[[length(fulldf_dm$cumSum1)]]

# Make ggplot
dmplot <- ggplot2::ggplot(
  data = fulldf_dm,
...
# Useful as there's no good way to include this on the plot in some instances
outdf_dm <- list(
...
if (sources$sourceX == "HYRAD") {
  outdf_dm$IDsX <- c("HYRAD")
  outdf_dm$WeightsX <- c(NA)
}

if (sources$sourceY == "HYRAD") {
  outdf_dm$IDsY <- c("HYRAD")
  outdf_dm$WeightsY <- c(NA)
}

# Pad with NA so have same lengths and avoid artefacts in the output file
max_length_dm <- max(length(raingauge_ids_dm$sourceX), length(raingauge_ids_dm$sourceY))

for (i in seq_along(outdf_dm)) {
  if (length(outdf_dm[[i]]) < max_length_dm) {
    n_na <- max_length_dm - length(outdf_dm[[i]])
    outdf_dm[[i]] <- c(outdf_dm[[i]], rep(NA, n_na))
  }
}
...
# Note: Check sign convention on a first
a_param_curves <- list(
  IMRD = c(0, 0.163, 0),
  WISKI = c(0, 0, 0)
)
...
# Initialise data frame for discontinuities
gapdf <- data.frame()
...
for (i in seq_len(n_ratings)) {
  ...
  for (j in seq_len(n_limbs_curve)) {
    ...
    h_limb <- limb_start + (0:n_per_limb) * (upper_level_curves[[i]][[j]] - limb_start) / n_per_limb
    h_curve <- c(h_curve, h_limb)
    
    # Make list of flow values
    q_limb <- c_param_curves[[i]][[j]] * (h_limb - a_param_curves[[i]][[j]])^b_param_curves[[i]][[j]]
    q_curve <- c(q_curve, q_limb)
    ...
    if (j >= 2) {
      ...
      gap_list <- c(gap_list, flow_list2[[j - 1]] - flow_list1[[j - 1]])
      
      # Find the level at which the limbs match
      if (gap_list[[length(gap_list)]] == 0.0) {
        match_list <- c(match_list, upper_level_curves[[i]][[j - 1]])
      } else {
        ...
        if (length(roots) == 0) {
          match_list <- c(match_list, NA)
        } else {
          match_list <- c(match_list, roots[[1]])
        }
      }
    }
  }
  ...
  df_rating <- rbind(df_rating, df_temp)
}
...
rating_plot <- ggplot2::ggplot(df_rating, aes(x = Q, y = H, colour = Rating, linetype = Rating)) +
  theme_bw()
...
rating_plot_log <- ggplot2::ggplot(logdf, aes(x = logQ, y = logH, colour = Rating, linetype = Rating)) +
  theme_bw()
...
if (include_gaugings) {
  ...
  gaugings_df_csv <- read.csv(paste0(base_spol_dir, "WISKI Measurements CSV files/1984topresent.csv"))
  
  # Set site number to enable gaugings to be read in
  site_gaugings <- subset.data.frame(
    gaugings_df_csv,
  ...
  if (include_remark_formatting) {
    site_gaugings$Remark <- str_replace_all(site_gaugings$Remark, "~r~n", ",")
  }
  
  # Assign Date column as date and then sort
  site_gaugings <- site_gaugings[order(as.POSIXct(site_gaugings$Date, format = "%d/%m/%Y")), ]
  ...
  gaugings_df <- subset.data.frame(site_gaugings, select = c(6, 5), header = TRUE)
  colnames(gaugings_df) <- c("Q", "H")
  
  # Add a "Rating" column to match format with df
  gaugings_df[, "Rating"] <- "Check\ngaugings"
  
  # Bind to df
  df_rating <- rbind(df_rating, gaugings_df)
...
if (is.na(wiski_export_level)) {
  ...
  # Reduce time range to match API spec
  if (is.na(level_data$data$dateTime[[length(level_data$data$dateTime)]] < datetime_end_rated)) {
    stop(paste("File", wiski_export_level, "has empty last row(s). Remove in text editor."))
             ...
             level_data$data$dateTime[[length(level_data$data$dateTime)]] < datetime_end_rated) {
    stop(paste("File", wiski_export_level, "does not span the entire date range."))
  } else {
    level_data$data <- level_data$data[
      level_data$data$dateTime >= datetime_start_rated &
        level_data$data$dateTime <= datetime_end_rated,
    ]
  }
}
...
if (is.na(wiski_export_flow)) {
  ...
  # Reduce time range to match API spec
  if (is.na(flow_data$data$dateTime[[length(flow_data$data$dateTime)]] < datetime_end_rated)) {
    stop(paste("File", wiski_export_flow, "has empty last row(s). Remove in text editor."))
             ...
             flow_data$data$dateTime[[length(flow_data$data$dateTime)]] < datetime_end_rated) {
    stop(paste("File", wiski_export_flow, "does not span the entire date range."))
  } else {
    flow_data$data <- flow_data$data[
      flow_data$data$dateTime >= datetime_start_rated &
        flow_data$data$dateTime <= datetime_end_rated,
    ]
  }
}
...
# Combine rated and observed flows into single data frame
...
# Combine rated and observed levels into single data frame
